...
const Admin = () => {
    ...
    const handleSubmit=()=>{
        ...
        axios.post("https://blue-frantic-springbok.cyclic.app/Data",{
            brand:veg,
            description:des,
            image:img,
            quantity:qua,
    ...
    const getData=()=>{
        ...
        axios.get("https://blue-frantic-springbok.cyclic.app/Data")
        .then(res=>{
            setPData(res.data)
        })
        .catch(err=>err)
    ...
    const handleDelete=(id)=>{
        setDummy(prev=>prev+1);
        alert(id)
        axios.delete(`https://blue-frantic-springbok.cyclic.app/Data/${id}`)
        .then(res=>{
            setDummy(prev=>prev+1);
            getData()
        })
    ...
    <div>
        ...
        <div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"20px",width:"80%",margin:"auto",marginTop:"20px"}}>
                    {pData.length>0 && pData.map((item,i)=>{
                        return <div >
                            <Box  textAlign={"center"} w={"auto"} border="3px dotted green" key={i}>
